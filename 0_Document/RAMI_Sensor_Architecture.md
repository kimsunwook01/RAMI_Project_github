...
### 5.1. Step 16: XML 센서 부착 및 시야(FOV) 지오메트리 시각화
- **작업 내용:** `rami_world.xml` 수정.
- **상세:** 
  - `gripper_camera`, `head_camera`, 8개의 `ultrasonic_sensor`, 1개의 `imu_sensor` 컴포넌트를 기존 메쉬 위치에 장착.
  - 2개의 반투명 노란색 사각뿔(카메라용), 8개의 반투명 하늘색 원뿔(초음파용)을 부착하여 물리 간섭 없이 렌더링.
