...
* **소프트웨어 아키텍처 설계서:** [RAMI_Software_Architecture.md](file:///d:/RAMI_project/0_Document/RAMI_Software_Architecture.md) (클린 4계층 구조 및 폴더 정의)
* **실내 공간 물리 환경 설계서:** [Phase4_Indoor_Physics_Setup.md](file:///d:/RAMI_project/0_Document/Phase4_Indoor_Physics_Setup.md) (URDF 변환, 마커 맵핑, 기구학 제약)
...
### 🛑 대기 중인 작업 (Todo)
- `[ ]` **Phase 6:** 문고리 하향 회전 및 동체 후진 연동형 문 개폐 FSM 상태 머신 구현 (Navigation + Manipulation 통합)

### ⏳ 진행 중인 작업 (In Progress)
- (현재 진행 중인 작업 없음)

### ✅ 완료된 작업 (Done)
- `[x]` **Phase 5:** 한국식 똑딱이 스위치 조작 구현 (OpenCV QR 인식, 비주얼 서보잉(Visual Servoing) 접근, 역기구학 기반 스위치 타격 FSM 완성) (2026-06-12)
- `[x]` **Phase 4:** 실내 공간 환경 최적화 (URDF 변환, 조명/마커 부착, 로봇-환경 간 물리 충돌 레이어(Collision Layer) 분리 안정화 완료) (2026-06-10)
- `[x]` **Phase 3:** 비전 시스템 및 센서 연동 구현 (YOLOv8 객체 탐지, 초음파, IMU Odometry) (완료: 2026-06-09)
  - `[x]` Step 16: XML 센서 부착 및 시야(FOV) 지오메트리 시각화 구현
  - `[x]` Step 17: 센서 프로세서 모듈 구현 (`vision`, `ultrasonic`, `imu`)
  - `[x]` Step 18: 센서 연동 통합 테스트 및 스트리밍 검증 (18-1 헤드 캠, 18-2 그리퍼 캠 완료)
...
- `[x]` **Phase 2 (Part 1):** 조인트 11단계 사전 검증 파이프라인 (상태 머 머신 기반 관절 제어 검증 및 전신 제어 인터페이스 구축 완료) (2026-06-09)