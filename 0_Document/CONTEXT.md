...
### 🛑 대기 중인 작업 (Todo)
- `[x]` **Phase 2:** 로봇 암 및 엔드 이펙터 기구학/제어 구현 (완료: 2026-06-09)
- `[진행중]` **Phase 3:** 손목 카메라 화각(FOV) 기반 규칙 기반 QR ID 식별 어댑터 구현 (`src/infrastructure/sensors/vision_processor.py`)
- `[ ]` **Phase 4:** 한국식 똑딱이 스위치 정면 접근 및 매니퓰레이터 말단 제어 조작 시나리오 테스트
- `[ ]` **Phase 5:** 문고리 하향 회전 및 동체 후진 연동형 문 개폐 FSM 상태 머신 구현

### ⏳ 진행 중인 작업 (In Progress)
- `[/]` **Phase 2 (Part 2):** 리프트 및 로봇암 조인트 자코비안 DLS 위치 제어(IK) 알고리즘 구현 (`src/domain/controllers/kinematics.py`)
